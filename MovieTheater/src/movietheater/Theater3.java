...
import static movietheater.mainMenu.firstTime3;

public class Theater3 extends javax.swing.JFrame {

    public ArrayList<JLabel> theater3Seat = new ArrayList<>();
    ...
    public double calculation(){
        int sum = 0;
        int cnt=0;
        JLabel label = theater3Seat.get(theater3Seat.size()-1);
            if(label.getName().substring(0,1).equals("A") || label.getName().substring(0,1).equals("B")){
                sum = 200;
            }
            else
                sum = 150;
        Starter.tick3.seatNo.setText(label.getName());
        Starter.tick3.price.setText(""+sum);
        Starter.tick3.showTime.setText(Starter.menu.jComboBox3.getSelectedItem().toString());
        if (sum == 150)
            Starter.tick3.seatType.setText("Normal");
        else if (sum == 200)  
            Starter.tick3.seatType.setText("Special");
        Starter.tick3.theater.setText("Theater3");
        Starter.tick3.movietitle.setText(Starter.menu.MTitle3.getText());          
        return sum;
    }
     public void Seat(){
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        for(int i = 0;i<theater3Seat.size();i++){
            if(theater3Seat.get(i).getBackground().equals(test1.getBackground())){
                cnt++;
            }
        }
        if(theater3Seat.size()==0 || cnt == 0 ){
            JOptionPane.showMessageDialog(null, "Please select your seat.");
        }
        else{
            for(JLabel label : theater3Seat){
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Theater3.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
